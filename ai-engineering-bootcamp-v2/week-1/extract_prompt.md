...
## Grade, IEP status, and attendance — current/record claims only
...
- **`medications`**: emit **one** fact per source that consolidates every named medication in that document (comma-separated). Do not emit a separate `medications` fact per mention ("Geodon…" then "hypothyroid medication" then "gum"). Explicit "no medications" / "none" is its own denial — do not mix `none` into a named-meds list.
...
8. Do not invent `grade`, `iep_status`, or `attendance` from future-plan labels, unfilled checkboxes, skill-area lists, or incidental cross-references (see above).
